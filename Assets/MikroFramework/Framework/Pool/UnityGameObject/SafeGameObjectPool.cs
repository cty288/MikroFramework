...
namespace MikroFramework.Pool
{
    public class SafeGameObjectPool:GameObjectPool {
        ...
        /// <summary>
        /// Instantitate a new SafeGameObjectPool to the game
        /// </summary>
        /// <returns></returns>
        public static SafeGameObjectPool Create(GameObject pooledPrefab, int initCount=0, int maxCount=50) {
            SafeGameObjectPool pool = new GameObject().AddComponent<SafeGameObjectPool>();
            pool.Init(pooledPrefab,initCount,maxCount);
            return pool;
        }
        ...
        private bool creatingObjs = false;
        private bool destroyingObjs = false;

        private void Update() {
            if (destroyedObjectInQueue.Count > 0 && !destroyingObjs) {
                destroyingObjs = true;
                StartCoroutine(DestoryObjectsInQueue());
            }

            if ((numHiddenObjectCreating > 0) && !creatingObjs) {
                creatingObjs = true;
